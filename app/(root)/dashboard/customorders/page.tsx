...
const Orders = () =>
{
    ...
    const [response, setResponse] = useState<PaginatedResponse<CustomOrder> | null>(null);
    const [config] = useState<OrderConfig>(mockData.customOrders.config);
    const [page, setPage] = useState(1);
    const [currentFilters, setCurrentFilters] = useState<Record<string, string>>({});
    const { isLoading, error, execute } = useFetch<PaginatedResponse<CustomOrder>>('/api/custom-order', {
        immediate: false
    });
    const { isLoading: isLoadingFile, error: errorFile, execute: executeFile } = useFetch('/api/custom-order/export', {
        immediate: false
    });
    ...
    useEffect(() =>
    {
        const fetchOrders = async () =>
        {
            ...
            const result = await execute({}, `/api/custom-order?${qs}`);
...
export default withAuth(Orders, { allowedRoles: [Role.ADMIN, Role.COURIER, Role.MANAGER, Role.MANUFACTURER] });
