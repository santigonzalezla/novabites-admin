...
const CategoryContent = ({ setData }: CategoryContentProps) =>
{
    const pathname = usePathname();
    const [isDisabled, setIsDisabled] = useState(true);
    const [formData, setFormData] = useState<CategoryProduct>();
    const [defaultData, setDefaultData] = useState<CategoryProduct>();
    const [categoryProducts, setCategoryProducts] = useState<Partial<Product>[]>([]);
    const categoryId = pathname.split('/').pop();
    const [modifiedFields, setModifiedFields] = useState<Partial<CategoryProduct>>();
    const { data, error, execute } = useFetch<CategoryProduct>(`/api/category-product/${categoryId}`);
    const { data: patchData, error: patchError, execute: patchExecute } = useFetch<CategoryProduct>(`/api/category-product/${categoryId}`, {
        method: 'PATCH',
        headers: {
